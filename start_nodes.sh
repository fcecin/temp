...
for i in $(seq 1 $NODE_COUNT); do
  ...
  # Start each node in a new terminal window and log to the respective directory
  gnome-terminal -- bash -c "./cometbft node --home $NODE_DIR --proxy_app=kvstore > $NODE_DIR/cometbft.log 2>&1"
done
